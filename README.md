...
- 地址簿分类可选择个人、自建或获授权的共享地址簿，并可按该地址簿的标签筛选；
- 按 ID、别名、主机名、用户名、平台和标签搜索；
- 批量收藏/取消收藏、清理最近连接，以及网格/列表视图切换；
- 地址簿别名、标签、设备在线状态和最后在线时间；
- 独立设置页包含常规、网络、显示、账户和关于，并持久化主题、语言、服务器、
  默认显示方式、图像质量、远程光标、音频与剪贴板选项；
...
Web Client V1 的视频解码器只实现 VP9，因此设置页会明确显示固定 VP9，不提供
不会生效的 VP8/AV1 选择。自适应码率对应 V1 的平衡图像质量默认值，其余显示与
会话选项会在建立新连接时写入 V1 的真实 `OptionMessage`。
